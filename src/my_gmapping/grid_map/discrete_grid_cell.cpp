...
void DiscreteCell::Update(const double probValue)
{
...
double DiscreteCell::RawToValue(const std::uint16_t discretizedValue)
{
...
std::uint16_t DiscreteCell::ValueToRaw(const double probValue)
{
    /* Ensure that the probability value is valid */
    assert(probValue >= 0.0 && probValue <= 1.0);
...
double DiscreteCell::ValueToOdds(const double probValue)
{
    /* Ensure that the probability value is valid */
    assert(probValue != Unknown);
    assert(probValue >= 0.0 && probValue <= 1.0);
...
double DiscreteCell::OddsToValue(const double oddsValue)
{
