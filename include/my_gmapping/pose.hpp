...
#ifndef MY_GMAPPING_POSE_HPP
#define MY_GMAPPING_POSE_HPP
...
struct RobotPose2D final
{
    ...
    /* Origin pose */
    static constexpr RobotPose2D<T> Zero { 0, 0, 0 };
...
/* Move backward
 * Calculate the pose such that Compound(pose, diffPose) becomes startPose */
template <typename T>
RobotPose2D<T> MoveBackward(const RobotPose2D<T>& startPose,
                            const RobotPose2D<T>& diffPose)
{
    T theta = startPose.mTheta - diffPose.mTheta;
    ...
    T x = startPose.mX - cosTheta * diffPose.mX + sinTheta * diffPose.mY;
    T y = startPose.mY - sinTheta * diffPose.mX - cosTheta * diffPose.mY;
